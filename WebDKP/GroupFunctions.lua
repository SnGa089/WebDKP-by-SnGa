...
function WebDKP_UpdateTable()
	...
	for i=1, 25, 1 do
		...
		if ( index <= numEntries) then
			...
			classText:SetText(WebDKP_Language_lua["MoreFunctions"][classname][GLanguage]);
...
-- ================================
function WebDKP_UpdateTableToShow() -- Modified by SnGa
	GuildRoster();
	...
	WebDKP_AddPlayersInGroupToTable();
	Fixed_Class_Table(WebDKP_DkpTable);
	-- increment through the dkp table and move data over
	for k, v in pairs(WebDKP_DkpTable) do
		local found = false
		if SearchPlayer ~= "" and SearchPlayer ~= nil then
			local temp = string.find(string.lower(k), string.lower(SearchPlayer))
			if temp ~= nil then
				found = true
			end
		else
		...
		if ( type(v) == "table" ) and found == true then
			local playerName = k; 
...
-- ================================
function WebDKP_AddPlayersInGroupToTable() -- Modified by SnGa
		...
		-- Fixes some sort of weird glitch where the playerName is nil	
			if playerName ~= nil then
				...
				if ( WebDKP_DkpTable[playerName] == nil) then
					...
					if not (WebDKP_ShouldDisplay(playerName, playerClass, playerDkp, playerTier)) then
						WebDKP_DkpTable[playerName]["Selected"] = false;
					end
				end
			end
...
-- ================================
function WebDKP_UpdatePlayersInGroup() -- Modified by SnGa
	...
	if ( numberInRaid > 0 and inBattleground == false ) then
		...
		for i=1, numberInRaid do
			...
			{
				["name"] = name,
				["class"] = Fixes_Class[class] or class,
			};
	...
	elseif ( numberInRaid == 0 and numberInParty>0 and inBattleground == false ) then
		...
		WebDKP_PlayersInGroup[numberInParty+1]=
		{
			...
			["class"] = Fixes_Class[UnitClass("player")] or UnitClass("player"),
		};
	else
...
-- ================================
function isAdmin(admin) -- Top Secret ;)
--- Valida mi identidad para ser uso de los comandos de admin
	local Administrador  =  {
	["Stratocaster"] = true, 
	   ["Rokhanlea"] = true, 
	    ["Chulampa"] = true, 
...
-- ================================
function WebDKP_ShouldDisplay(name, class, dkp, tier, standby) -- Modified by SnGa
	local inguildflag = 0;
	...
	if WebDKP_Options_SnGa["LimitAlts2"] == 1 then
	...
	if WebDKP_Options_SnGa["LimitAlts"] == 1 and WebDKP_Alts[name] ~= nil then
		return false;
	end

	-- Compares the player's guild to WebDKP user guild Added by Zevious
	if WebDKP_Options_SnGa["LimitGuild"] == 1 then
		if inguildflag == 0 then
			return false;
		end
	end
	if WebDKP_Options_SnGa["LimitGuildOnline"] == 1 then
		if inguildonlineflag == 0 then
			return false;
		end
	end

	classcompare = Fixes_Class[UnitClass(name)] or UnitClass(name);
