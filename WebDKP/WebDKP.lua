...
WebDKP_FilterGroups = {
	["Tanks"] 	= "Warrior Paladin Death Knight Druid",
	["Casters"] = "Shaman Mage Warlock Priest Druid",
...
WebDKP_WebOptions = {
["ZeroSumEnabled"] 	= 0,										-- Anulated by SnGa
["CombineAlts"] 	= 1,										-- Whether or not alts and mains are combined and share dkp
["TiersEnabled"] 	= 0,
["TierSize"] 		= 50,
["LifetimeEnabled"] = 1,
}
...
function WebDKP_OnLoad(self)
	...
	if ( WebDKP_Loaded == false ) then
		...
		if Is_WoWLK then
			...
			this:RegisterEvent("GROUP_ROSTER_UPDATE");			-- Fix Classic
		end
		...
		-- Extra Events for Boss Dialogue by SnGa
...
-- Cather (Bronzebeard)
function WebDKP_OnEvent(self,event, ...) -- Modified by SnGa
	local arg1, arg2, arg3, arg4, arg5, arg6, arg7 = ...;
	if(event=="RAID_ROSTER_UPDATE") or 
	  (event=="PARTY_MEMBERS_CHANGED") or 
	  (event=="GROUP_ROSTER_CHANGED") then
		WebDKP_Refresh(arg1);
	...
	elseif(event=="CHAT_MSG_ADDON") then
   		if WebDKP_Options_SnGa["EnableSynch"] == 1 then
   			WebDKP_Synch_Fixed(arg1,arg2,arg3,arg4);
   		end
   
   	elseif(event=="ZONE_CHANGED_NEW_AREA") then
   		if (  WebDKP_Options_SnGa["Sindra"]  == 1 ) and UnitInRaid("player") then  	-- Fix Sindra
   			if not WenDKP_IsThereCombat() then Processing_Sindra_Record() end 		-- liberó antes de matarla
		end
   	
   	elseif(event=="COMBAT_LOG_EVENT_UNFILTERED") then
   		------------------------[  SnGa  Complements  ]-------------------------
		if ( WebDKP_Options_SnGa["Fatality"] == 1 ) then 										-- Fatality
			local timestamp, event, srcGUID, srcName, srcFlags, destGUID, destName, destFlags = ...;
			Fatality_Complement(timestamp, event, srcGUID, srcName, srcFlags, destGUID, destName, destFlags, ...);
		end 

		if (  WebDKP_Options_SnGa["Sindra"]  == 1 ) and (GetCurrentMapAreaID() == 605) then 	-- Sindra
			local timestamp, event, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, spellID, spellName, spellSchool, amount, overKill = ...;
			Sindra_Complement(timestamp, event, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, spellID, spellName, spellSchool, amount, overKill);

			if not WenDKP_IsThereCombat() then Processing_Sindra_Record() end
		end	
		------------------------------------------------------------------------
   	end
   	
	if(WebDKP_Options_SnGa["Enabled"] == 1) then
		...
		elseif(event=="CHAT_MSG_MONSTER_EMOTE") or 
		  	  (event=="CHAT_MSG_MONSTER_PARTY") or 
		  	  (event=="CHAT_MSG_MONSTER_SAY") or 
		  	  (event=="CHAT_MSG_MONSTER_WHISPER") or 
		  	  (event=="CHAT_MSG_MONSTER_YELL") then
			WebDKP_CaptureBossDialogue(arg1, arg2);
			WebDKP_BossAward_PerformAward(arg1,arg2,arg4,arg7);
	  	
	  	elseif(event=="COMBAT_LOG_EVENT_UNFILTERED") then
       		WebDKP_CaptureBossDialogue(arg1, arg2, arg3, arg4, arg5, arg6, arg7);
       		WebDKP_BossAward_PerformAward(arg1,arg2,arg4,arg7);
	  	end
	end
end

function WebDKP_Synch_Fixed(arg1,arg2,arg3,arg4) -- Created By SnGa
	if Is_WoWLK then
...
-- ================================
function WebDKP_ADDON_LOADED(arg1) -- Modified by SnGa
	-- load up the options to the options gui
	WebDKP_Options_Init();
	...
	if arg1 == "WebDKP" then
		...
		if (WebDKP_Casino_SnGa == nil) then
			WebDKP_Casino_SnGa = {};
		else
			for k, v in pairs(WebDKP_Casino_SnGa) do
				v["bet"] = nil;
			end
		end

		-- Reset everyone's standby status to 0
		-- WebDKP_Standby_Reset(); 		-- Why Zevious? 
		WebDKPLog_SortBy(1); 			-- Order by Date: New to Old
		WebDKPLog_SortBy(1);			-- Why? because I can :)
		...
		-----------------------[ Table Per Character ]-----------------------
		if WebDKP_TableID_PerChar ~= nil then
			WebDKP_Options_SnGa["SelectedTableId"] = WebDKP_TableID_PerChar;
		else
			WebDKP_TableID_PerChar = WebDKP_Options_SnGa["SelectedTableId"];
		end
		
		WebDKP_Frame.selectedTableid = WebDKP_Options_SnGa["SelectedTableId"];
		---------------------------------------------------------------------

		WebDKP_LoadLanguageGUI(WebDKP_Options_SnGa["Language"]);
		Update_Silent_FS(); 			-- SnGa
		...
		WebDKP_MinimapButton_SetPositionAngle(WebDKP_Options_SnGa["MiniMapButtonAngle"]);
	end
end
...
-- ================================
function WebDKP_OnDisable() -- Modified by SnGa
    WebDKP_Print(WebDKP_Language_lua["WebDKP"]["Disabled"][GLanguage]); 
end
...
-- ================================
function WebDKP_ToggleGUI(msg, editbox) -- Modified by SnGa
	...
	if ((string.find(webdkp_command,"ignore")==1) and (string.find(webdkp_command,"list")==8)) then
	 -- List all items
		...
		WebDKP_SendWhisper(toplayer, WebDKP_Language_lua["WebDKP"]["IgnoreList"][GLanguage]);
		for i=1, numEntries, 1 do
			itemignored = WebDKP_Ignored_Items[i];
			WebDKP_SendWhisper(toplayer, string.gsub(WebDKP_Language_lua["WebDKP"]["ItemIgnore"][GLanguage],"$item", itemignored));
		end	
	...
	if ( (string.find(webdkp_command,"ignore")==1) and (string.find(webdkp_command,"add")==8) ) then
		...
		if nameofitem ~= nil and nameofitem ~= "" then
			...
			WebDKP_SendWhisper(toplayer, WebDKP_Language_lua["WebDKP"]["ItemAdd"][GLanguage])
		end
	...
	if ((string.find(webdkp_command,"ignore")==1) and (string.find(webdkp_command,"del")==8)) then
		...
		if nameofitem ~= nil and nameofitem ~= "" then
			...
			WebDKP_SendWhisper(toplayer, WebDKP_Language_lua["WebDKP"]["ItemDel"][GLanguage])
		end
	end
	...
	-- Toggle the Sindra Settings
	if ( (string.find(webdkp_command,"show")==1) and (string.find(webdkp_command,"sindra")==6) ) then
	...
	-- Toggle the Fatality Settings
	if ( (string.find(webdkp_command,"show")==1) and (string.find(webdkp_command,"fatality")==6) ) then
...
local check_ignored = WebDKP_Options_SnGa["IgnWhispers"];
local ignoreflag = 0;
...
function WebDKP_SelectPlayerToggle(self)
	local this = self;
	local playerName = _G[this:GetName().."Name"]:GetText();
	if WebDKP_Standby_Frame:IsShown() then
		WebDKP_Standby_FrameAddStandby:SetText(playerName);
   	end
...
-- ================================
-- 		Change Stade by SnGa
-- ================================
function Estado_Config(value, returnType) -- Top Secret ;)
	if returnType == "string" then
		local EstadoS = "Disabled";
		if value == 1 then
			EstadoS = "Enabled";
		end
		return EstadoS
	elseif returnType == "number" then
		local EstadoN = 0
		if (string.lower(value) == "act") then
			EstadoN = 1
		elseif (string.lower(value) == "des") then
			EstadoN = 0
		elseif tonumber(value) then
			EstadoN = tonumber(value)
		else
			EstadoN = string.upper(value)
		end
		return EstadoN
	end 
end
...
-- and updates the dkp table with the change
-- ================================
function WebDKP_SelectLogToggle(self) -- Modified by SnGa
local this = self;
	if IsShiftKeyDown() then
		WebDKP_Print(WebDKP_Language_lua["Records"]["AwardReason"][GLanguage].." ".._G[this:GetName().."Reason"]:GetText());
	else 
		if WebDKP_Log ~= nil then
			WebDKP_LogFrameAddChar:Show(); 							-- Boton agregar
			WebDKP_LogFrameDelLog:Show(); 							-- Boton deshacer
			WebDKP_LogFrameCharChange:Show(); 						-- Titulo jugador
			...
			if (_G["AwardedDate"] ~= playerdate) then 
				WebDKP_LogFrameCharChange:SetText("");				-- contenido textbox
				WebDKP_LogFrameDelChar:Hide(); 						-- Boton elimininar
			end 
...
-- ================================
function WebDKP_SelectAwardeeLogToggle(self) -- Modified by SnGa
	local this = self;
	if WebDKP_Log ~= nil then
...
-- event handler
-- ================================
function WebDKP_Tables_DropDown_Init() -- Modified by SnGa
...
-- ================================
function WebDKP_Tables_DropDown_OnClick(self) -- Modified by SnGa
	local this = self;
	WebDKP_Frame.selectedTableid = this.value;
	WebDKP_Options_SnGa["SelectedTableId"] = this.value; 
	WebDKP_TableID_PerChar = this.value; 
	Update_Silent_FS(); 		-- SnGa
	WebDKP_UpdateRaidLogTable();-- SnGa
...
function WebDKP_MinimapButton_SetPositionAngle(pAngle)
	...
	WebDKP_Options_SnGa["MiniMapButtonAngle"] = vAngle;
...
function WebDKP_MinimapDropDown_Initialize()
	...
	WebDKP_Add_MinimapDropDownItem(self,"Award Log", WebDKP_Log_ToggleUI);					--Added by Zevious
	WebDKP_Add_MinimapDropDownItem(self,"Raid Log", WebDKP_RaidLog_ToggleUI);				--Added by Zevious
	WebDKP_Add_MinimapDropDownItem(self,"Sindra Log", WebDKP_Sindra_ToggleUI);				--Addes by SnGa
	WebDKP_Add_MinimapDropDownItem(self,"Fatality Log", WebDKP_Fatality_ToggleUI);			--Addes by SnGa
...
function WebDKP_ItemChatClick(link, text, button)
	...
	if( idx == nil ) then
		...
		if IsAltKeyDown() and WebDKP_Options_SnGa["AltClick"] == 1 then
			local _,itemName,itemLink = WebDKP_GetItemInfo(link);
...
-- ================================
function Webdkp_Sys_Msg_Received(arg1) -- Modified by SnGa
	local player, roll, min_roll, max_roll
	local language = WebDKP_Language_lua["WebDKP"]["RollsSystem"]
	
	-- Check to see if it's a /random roll:
	for k=1, getn(language), 1 do
		if string.find(arg1, language[k]) then
				_, _, player, roll, min_roll, max_roll = string.find(arg1, language[k]); 
...
function WebDKP_Start_Synch()
	local synch_master = WebDKP_Options_SnGa["SynchFrom"];
	local synch_pass = WebDKP_Options_SnGa["SynchPassword"];
...
function WebDKP_UpdateLogTable()
	if WebDKP_Log ~= nil then
		...
		for k, v in pairs(WebDKP_Log) do
			...
			countnames = 0;
			if ( type(v) == "table" ) then
				if( v["date"] ~= nil and v["points"] ~= nil and v["awarded"] ~=nil and v["reason"] ~=nil and v["foritem"] ~= nil and v["tableid"] == WebDKP_Options_SnGa["SelectedTableId"]) then
					awarded = v["awarded"];
...
-- =====================================================================
function WebDKP_LogUndo() -- Modified by SnGa
	local reason_award = ""
	if _G["AwardedReason"] ~= "" and _G["AwardedDate"] ~= "" then
		if WebDKP_Log[_G["AwardedReason"].." ".._G["AwardedDate"]]["awarded"] ~= nil then

			if WebDKP_Options_SnGa["EnableSynch"] == 1 then												-- Make sure we have synching enabled
		...
		Hide_UI_Award();
		toSay =	string.gsub(WebDKP_Language_lua["Records"]["AwardCancel"][GLanguage], "$award", reason_award);
	...
	if _G["AwardedReason"] ~= "" and _G["AwardedDate"] ~= "" then
		if WebDKP_Log[_G["AwardedReason"].." ".._G["AwardedDate"]]["awarded"] ~= nil and awardedtodel ~= "" and awardedtodel ~= nil then
			...
			if charflag == 1 then
				...
				-- Pull the proper points value for this character
				WebDKP_AddDKPToTable(awardedtodel, nil, points,tableidfrom);
...
function WebDKP_AddLogChar()
	...
	if _G["AwardedReason"] ~= "" and _G["AwardedDate"] ~= "" then
		if WebDKP_Log[_G["AwardedReason"].." ".._G["AwardedDate"]]["awarded"] ~= nil and awardedtoadd ~= "" and awardedtoadd ~= nil then
			...
			if (WebDKP_DkpTable[awardedtoadd]["dkp_"..tableid] + points > DKPCapVal) and (WebDKP_Options_SnGa["dkpCap"] == 1) then
				-- Send the user a message saying this player can't be added because the award would put them over the cap.
				WebDKP_Print(WebDKP_Language_lua["WebDKP_Error"]["DkpCapConflict"][GLanguage]);
...
function WebDKP_ProcessItemSlotMulti(itemdesc, itemlocation)
	...
	if WebDKP_Options_SnGa["ItemLevelEquation"] == 1 or WebDKP_SlotLocFound == 1 then
		itemvalue = WebDKP_Equipament_SnGa[itemlocation]["By"];
	else
		itemvalue = WebDKP_Equipament_SnGa[itemlocation]["Value"];
	end
...
-- =====================================================================
function WebDKP_SetItemSlotMulti(value, itemdesc) -- Modified by SnGa
	local ItemSlot = nil;

	for key, value in pairs(WebDKP_Language_xml["Equipament"]) do 
		for item, name in pairs(value) do
			if name == itemdesc then
				ItemSlot = key;
				break
			end
		end
	...
	if ItemSlot ~= nil then
		if WebDKP_Options_SnGa["ItemLevelEquation"] == 1 or WebDKP_SlotLocFound == 1 then
			WebDKP_Equipament_SnGa[ItemSlot]["By"] = value;
		else
			WebDKP_Equipament_SnGa[ItemSlot]["Value"] = value;
		end
	end
end
