	...
	if WebDKP_RaidInfo ~= nil then
		...
		for k, v in pairs(WebDKP_RaidInfo) do
			...
			if ( type(v) == "table" ) then
				if( v["raid"] ~= nil and v["date"] ~= nil and v["starttime"] ~= nil and v["endtime"] ~= nil and v["totaltime"] ~= nil and v["completedate"] ~= nil and v["tableid"] == WebDKP_Options_SnGa["SelectedTableId"]) then
...
function WebDKP_UpdateCharRaidLogTable()
	...
	if WebDKP_CharRaidInfo ~= nil then
		local entries = { };
		if WebDKP_Options_SnGa["InGroup"] == 1 then
...
function WebDKP_RaidStart()
	...
	WebDKP_Options_SnGa["GlobalStartTime"] = startTime2; 
	WebDKP_Options_SnGa["GlobalRaid"] = raid;
	...
	WebDKP_RaidInfo[raid.." "..startTime2]["tableid"] = WebDKP_Options_SnGa["SelectedTableId"];
	...
	WebDKP_RaidInfo[raid.." "..startTime2]["totalplayers"] = Count;
	-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	-- Adds the raid info for the character raid log
	-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	WebDKP_Print(string.gsub(WebDKP_Language_lua["RaidAttendance"]["RStartPrint"][GLanguage], "$Start", startTime));

	WebDKP_RaidStatus( "Start" );
	WebDKP_UpdateRaidLogTable();
...
-- =================================================================
function WebDKP_RaidEnd() -- Modified by SnGa
	if WebDKP_Options_SnGa["GlobalStartTime"] ~= "" and WebDKP_Options_SnGa["GlobalRaid"] ~= "" then
		...
		_,_,hoursb,minutesb,secondsb = string.find(WebDKP_Options_SnGa["GlobalStartTime"], pattern);			-- Setup values for start times to calc total
		...
		-- Makes sure there is an entry for this raid
		if (WebDKP_RaidInfo[WebDKP_Options_SnGa["GlobalRaid"].." "..WebDKP_Options_SnGa["GlobalStartTime"]]) then
			WebDKP_RaidInfo[WebDKP_Options_SnGa["GlobalRaid"].." "..WebDKP_Options_SnGa["GlobalStartTime"]]["endtime"] = endTime;
			WebDKP_RaidInfo[WebDKP_Options_SnGa["GlobalRaid"].." "..WebDKP_Options_SnGa["GlobalStartTime"]]["totaltime"] = totaltimeraid;
		end

		local temp = WebDKP_Language_lua["RaidAttendance"]["REndPrint"][GLanguage];
		temp = string.gsub(temp, "$endTime", endTime )
		temp = string.gsub(temp, "$totaltimeraid", totaltimeraid )
		WebDKP_Print(temp);

		WebDKP_RaidStatus( "Finish" );
		WebDKP_Options_SnGa["GlobalStartTime"] = "";
		WebDKP_Options_SnGa["GlobalRaid"] = "";
		WebDKP_Standby_Reset(); -- Better here Zevious! :) 
		WebDKP_UpdateRaidLogTable();
	end
end
...
function WebDKP_ScrollRaidLogListToggle()
	...
	if WebDKP_RaidInfo ~= nil then
		-- Set the previous selection to not selected
		if WebDKP_Options_SnGa["GlobalStartTime"] ~= "" and WebDKP_Options_SnGa["GlobalRaid"] ~= "" then
			WebDKP_RaidInfo[WebDKP_Options_SnGa["GlobalRaid"].." "..WebDKP_Options_SnGa["GlobalStartTime"]]["selected"] = false;
		end
		if _G["LineLocation"] ~= "" then
			_G["LineLocation"]:SetVertexColor(0, 0, 0, 0);
		end
...
-- ====================================================================
function WebDKP_DeleteRaid() -- Modified by SnGa
	if previous_starttime == "" then return end
	if WebDKP_RaidInfo[previous_raid.." "..previous_starttime]["endtime"] ~= "" then
		if previous_raid ~= "" and previous_starttime ~= "" then
		...
		WebDKP_Print(WebDKP_Language_lua["WebDKP_Error"]["NoDelete"][GLanguage])
	end
end
...
-- ====================================================================
function WebDKP_EndTime() -- Modified by SnGa
	if previous_starttime == "" then return end
	if WebDKP_RaidInfo[previous_raid.." "..previous_starttime]["endtime"] ~= "" then
		...
		WebDKP_Print(WebDKP_Language_lua["WebDKP_Error"]["NoFinished"][GLanguage])
	end
end
...
function WebDKP_ApplyDateFilter(filterdate)
	if filterdate == nil or filterdate == "" then
		WebDKP_Print(WebDKP_Language_lua["RaidAttendance"]["Filters"][GLanguage])
		return nil;
	else
		local pattern = "(%d+)-(%d+)-(%d+)";
