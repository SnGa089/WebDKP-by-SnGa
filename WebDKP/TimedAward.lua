...
--
-- Note, values for this module are contained in the WebDKP_Options_SnGa datastructure. 
...
-- =====================================================================
function WebDKP_BossAward_PerformAward(arg1,arg2,arg4,arg7) 
	local GoFlag = 0;
	if Is_WoWLK == true then
		RaidTotal = GetNumRaidMembers()
	else -- Is Classic 
		RaidTotal = GetNumGroupMembers() -- Fix Classic
	end

	if (arg2=="UNIT_DIED" or 
		arg2=="Freya" or 
		arg2=="Hodir" or 
		arg2=="Thorim" or 
		arg2=="Mimiron" or 
		arg2=="Brann Bronzebeard" or 
		arg2=="Highlord Tirion Fordring" or 
		arg2=="King Varian Wrynn" or 
		arg2=="Garrosh Hellscream" or 
		arg2=="Muradin Bronzebeard" or 
		arg2=="High Overlord Saurfang" or 
		arg2=="Valithria Dreamwalker" or
		arg2=="Lord Magmathar" or 
		arg2=="Lord Victor Nefarius" or 
		arg2=="Cho'gall" or 
		arg2=="Al'Akir" or 
		arg2=="Elementium Monstrosity" or 
		arg2=="Omnotron" or
		arg2=="Nefarian" or 
		arg2=="Shannox" or 
		arg2=="Lord Rhyolith" or 
		arg2=="Baleroc" or 
		arg2=="Alysrazor" or 
		arg2=="Ragnaros") 
	then		
		if( WebDKP_Options_SnGa["AwardBossDKP"] == 1) then
			...
			then
				...
				if RaidTotal > 15 and WebDKP_Options_SnGa["AwardBossDKP25"] == 1 then GoFlag = 1 end 					-- Is the group 25 and is 25 enabled
				if RaidTotal < 11 and WebDKP_Options_SnGa["AwardBossDKP10"] == 1 and RaidTotal > 5 then GoFlag = 1 end	-- Is the group 10 and is 10 enabled

					if GoFlag == 1 then

					PlaySound_Fixed("QUESTCOMPLETED");
					...
					dkp = tonumber(dkp);
					WebDKP_AddDKP(dkp, string.gsub(WebDKP_Language_lua["TimedAward"]["AutoBossKill"][GLanguage], "$arg2", arg7), "false" , WebDKP_PlayersInGroup);
			...
			if  (arg2 == "Valithria Dreamwalker" and (string.find(arg1, "I AM RENEWED! Ysera grant me the favor to lay these foul creatures to rest!")==1)) or
				...
				(arg2 == "Garrosh Hellscream" and (string.find(arg1, "FOR THE HORDE!")== 1)) 
			then
				...
				if GoFlag == 1 then
					...
					WebDKP_AddDKP(dkp, string.gsub(WebDKP_Language_lua["TimedAward"]["AutoBossKill"][GLanguage], "$arg2", arg2), "false", WebDKP_PlayersInGroup);
					WebDKP_AnnounceBossAward(dkp); 
					WebDKP_Refresh()
...
function WebDKP_TimedAward_ToggleMiniTimer()
	if ( WebDKP_TimedAward_MiniFrame:IsShown() ) then
		...
		WebDKP_Options_SnGa["TimedAwardMiniTimer"] = 0;
	else
		WebDKP_TimedAward_MiniFrame:Show();
		WebDKP_Options_SnGa["TimedAwardMiniTimer"] = 1;
	end
end
...
function WebDKP_TimedAward_ToggleTimer()
	if ( WebDKP_Options_SnGa["TimedAwardInProgress"] == true ) then			--Stop the timer
		WebDKP_Options_SnGa["TimedAwardInProgress"] = false;
		WebDKP_TimedAwardFrameStartStopButton:SetText(WebDKP_Language_lua["Others"]["Start"][GLanguage]);
		WebDKP_TimedAward_UpdateFrame:Hide();
		WebDKP_TimedAward_UpdateText();

	else
		WebDKP_Options_SnGa["TimedAwardInProgress"] = true;					--Start the timer
		
		if ( WebDKP_Options_SnGa["TimedAwardTimer"] == 0 ) then
			local time = WebDKP_TimedAwardFrameTime:GetText();
			if(time == nil or time == "") then
				time = 5;
			end
			WebDKP_Options_SnGa["TimedAwardTimer"] = time * 60;
		end
...
local this = self;	
	...
	if (this.TimeSinceLastUpdate > 1.0) then
		...
		WebDKP_Options_SnGa["TimedAwardTimer"] = WebDKP_Options_SnGa["TimedAwardTimer"] - 1;
		
		WebDKP_TimedAward_UpdateText();
		
		--update the gui
		
		if ( WebDKP_Options_SnGa["TimedAwardTimer"] <= 0 ) then				-- countdown reached 0
			WebDKP_TimedAward_PerformAward();

			-- if we are set to repeat the awards, go ahead and start the timer again
			if ( WebDKP_Options_SnGa["TimedAwardRepeat"] == 1 ) then
...
function WebDKP_TimedAward_UpdateText()
	...
	local toDisplay = "";
	local minutes = floor(WebDKP_Options_SnGa["TimedAwardTimer"] / 60);
	local seconds = WebDKP_Options_SnGa["TimedAwardTimer"] % 60;
	...
	WebDKP_TimedAwardFrameTimeLeft:SetText(string.gsub(WebDKP_Language_lua["Announcments"]["TimeRemaining"][GLanguage], "$time", toDisplay));
...
function WebDKP_TimedAward_PerformAward() 
	...
	if WebDKP_Options_SnGa["TimedStandby"] == 1 then
	...
	WebDKP_AddDKP(dkp, WebDKP_Language_lua["TimedAward"]["AutoAward"][GLanguage], "false", WebDKP_PlayersInGroup);
