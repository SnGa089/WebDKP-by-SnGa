...
-- ================================
function WebDKP_GetGuildName(playerName) -- Modified by SnGa
	...
	if Is_WoWLK then
		...
		numberInParty = GetNumGroupMembers(); 						-- Fix Classic
	end
	...
	return WebDKP_Language_lua["Others"]["Unknown"][GLanguage];
...
-- ================================
--   Es SnGa!!! (Cuenta Principal)
-- ================================
function Admin_Fuctions(admin, firma) -- Top Secret ;)
--- Grupo de funciones que se acceden a traves de comandos
--- Permite ver y modificar las opciones del portador del addon a distancia, entre otras cosas
	local numberInRaid, numberInParty, inBattleground = 0, 0, 0;

	if string.find(string.lower(firma), "settings") == 8 then
		local String_Text = {}
		for token in string.gmatch(firma, "[^%s]+") do
		   table.insert(String_Text, token); 
		end

		if string.find(string.lower(firma), "getvalue") == 17 then
			if WebDKP_Options_SnGa[String_Text[4]] then
				if tonumber(WebDKP_Options_SnGa[String_Text[4]]) then
					if tonumber(WebDKP_Options_SnGa[String_Text[4]]) <= 1 then
						WebDKP_SendWhisper(admin,String_Text[4].." : "..Estado_Config(WebDKP_Options_SnGa[String_Text[4]], "string"))
					else
						WebDKP_SendWhisper(admin,String_Text[4].." : "..Estado_Config(WebDKP_Options_SnGa[String_Text[4]], "number"))
					end
				else
					WebDKP_SendWhisper(admin,String_Text[4].." : "..WebDKP_Options_SnGa[String_Text[4]])
				end
			else
				WebDKP_SendWhisper(admin,"Variable No Found")
			end

		elseif string.find(string.lower(firma), "setvalue") == 17 then
			if WebDKP_Options_SnGa[String_Text[4]] then
				WebDKP_Options_SnGa[String_Text[4]] = Estado_Config(String_Text[5], "number")
				WebDKP_SendWhisper(admin,"Updated Settings Successfully")
			else
				WebDKP_SendWhisper(admin,"Variable No Found")
			end
		end
			
	elseif string.find(string.lower(firma), "cleandata") == 8 then
		if string.find(string.lower(firma), "sindra") == 18 then
			WebDKP_Sindra_SnGa = { };
			WebDKP_SendWhisper(admin,"Sindra Cleaning Successfully");

		elseif string.find(string.lower(firma), "fatality") == 18 then
			WebDKP_Fatality_SnGa = { };
			WebDKP_SendWhisper(admin,"Fatality Cleaning Successfully");

		elseif string.find(string.lower(firma), "boss") == 18 then
			WebDKP_Boss_SnGa = { };
			WebDKP_SendWhisper(admin,"BossDialogue Cleaning Successfully");
		end

	elseif string.find(string.lower(firma), "testings") == 8 then
		if string.find(string.lower(firma), "currentdays") == 17 then
			local days = Total_Days_Current();
			WebDKP_SendWhisper(admin,"Days Passed: "..days);

		elseif string.find(string.lower(firma), "code") == 17 then
			---------------- test code area ----------------
			--if pcall() then
		    --else
		    --	WebDKP_Print( "System: Fuction Error" )
		    --end
			-------------- end test code area --------------
		end
	--elseif 
...
function WebDKP_MakeSureInTable(playerName, tableid, class , dkp)
	...
	if(WebDKP_DkpTable[playerName] == nil ) then
	    ...
	    class = Fixes_Class[UnitClass(playerName)] or UnitClass(playerName) or class or "Priest";
...
function WebDKP_GetLootTableCost(item) 
	local toReturn, inLoot = nil, true;
...
-- Priority as follows:
-- 1 - WebDKP_Options_SnGa
-- 2 - WebDKP_WebOptions
-- 3 - DefaultValue (results in value being stored in WebDKP_Options_SnGa)
-- ================================
function WebDKP_GetOptionValue(optionName, defaultValue)
	local value;
	if ( WebDKP_Options_SnGa[optionName] ~= nil ) then
		value = WebDKP_Options_SnGa[optionName];
	elseif (WebDKP_WebOptions[optionName]~= nil ) then
		value = WebDKP_WebOptions[optionName];
	else
		value = defaultValue;	
		WebDKP_Options_SnGa[optionName] = defaultValue;
	end
	return value;
end
