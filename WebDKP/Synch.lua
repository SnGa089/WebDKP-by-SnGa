...
function WebDKP_Synch_Processing(arg1,arg2) 
	...
	local enabled = WebDKP_Options_SnGa["EnableSynch"];
	local masterorbackup = WebDKP_Options_SnGa["MasterOrBackup"];
			...
			elseif (string.find(command,"!WDKP Mult End")==1) then
				...
				-- ====================== Process Other Multi Person Awards ==========================
					...
					for i=1, glob_awards[senderName]["numawarded"], 1 do
							...
							if WebDKP_Options_SnGa["dkpCap"] == 1 and (WebDKP_DkpTable[name]["dkp_"..tableid] + glob_awards[senderName]["dkp"] > DKPCapVal) then
